...
def usage(script, msg):
    ...
    sys.stderr.write(f"""
Usage:
  {script} [--dpi N] [basename] > out.pdf
  {script} [--dpi N] -s [page.jb2]... > out.pdf
  ...
  --dpi N: set DPI for page size calculation (default: 72)
...
def parse_args(script: str) -> tuple:
    """Parses command-line arguments and returns the symbol table, page files, and DPI."""
    global dpi

    # Check for --dpi argument
    args = sys.argv[1:]
    if "--dpi" in args:
        dpi_idx = args.index("--dpi")
        if dpi_idx + 1 < len(args):
            try:
                dpi = int(args[dpi_idx + 1])
            except ValueError:
                pass
            # Remove --dpi and its value from args
            args = args[:dpi_idx] + args[dpi_idx+2:]

    if "-s" in args:
        # Standalone mode, no global symbol table
        pages = [arg for arg in args if arg != "-s"]
        return "", pages
    elif len(args) == 1:
        base_name = args[0]
        sym = f"{base_name}.sym"
